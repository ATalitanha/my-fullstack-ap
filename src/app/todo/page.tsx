...
export default function TodosPage() {
	...
	const { t } = useTranslation("app.todo");
	...
	const saveTodo = async () => {
		if (!title.trim() || !token) {
			showResponse({ text: `❌ ${t("error_title")}`, type: "error" });
			return;
		}
	...
	const confirmDelete = async () => {
		...
		try {
			...
			const data = await res.json();
			if (res.ok)
				showResponse({ text: `✅ ${t("success_delete")}`, type: "success" });
			else
				showResponse({
					text: `❌ Error: ${data.message || "Failed"}`,
					type: "error",
	...
	const confirmComplete = async () => {
		...
		try {
			...
			const data = await res.json();
			if (res.ok)
				showResponse({ text: `✅ ${t("success_complete")}`, type: "success" });
			else
				showResponse({
					text: `❌ Error: ${data.message || "Failed"}`,
					type: "error",
	...
	return (
					...
					<div className="grid lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
							...
							<div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-3xl shadow-2xl p-8">
								<div className="flex items-center gap-3 mb-6">
									...
									<h2 className="text-2xl font-bold text-gray-800 dark:text-white">
										{editingTodo ? t("edit_todo") : t("add_todo")}
									</h2>
								</div>
                      ...
                      disabled:opacity-60 disabled:cursor-not-allowed"
						...
						<motion.div
							...
							className="lg:col-span-1"
						>
							<div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-3xl shadow-2xl0 h-full">
								<div className="p-6 bg-linear-to-r from-red-500/10 via-gray-50 to-blue-500/10 dark:from-red-500/40 dark:via-gray-700/40 dark:to-blue-500/40 rounded-t-3xl">
        ...
        hover:scrollbar-thumb-blue-500/90 dark:hover:scrollbar-thumb-blue-500/80"
								>
									{loading ? (
										...
										>
											...
											<p>{t("no_tasks")}</p>
											<p className="text-sm mt-1">{t("first_task")}</p>
										</motion.div>
									) : (
			...
			<ConfirmModal
				...
				confirmText={t("confirm_delete_btn") || "Delete"}
			...
			<ConfirmModal
				...
				message={t("confirm_complete")}
				confirmText={t("confirm_complete_btn") || "Complete"}
			...
			<ConfirmModal
				...
				message={t("confirm_uncomplete")}
				confirmText={t("confirm_uncomplete_btn") || "Uncomplete"}
			...
			<ConfirmModal
				...
				message={t("confirm_edit")}
				confirmText={t("confirm_edit_btn") || "Edit"}
				confirmColor="bg-yellow-500 hover:bg-yellow-600"
			/>
		</>
	);
