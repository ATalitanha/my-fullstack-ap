...
describe('Login API', () => {
  it('should log in a user and return an access token', async () => {
    ...
    (loginSchema.safeParse as vi.Mock).mockReturnValue({ success: true, data: { username: 'test', password: 'password' } });
    ...
    expect(response.cookies.get('refreshToken')?.value).toBe(mockTokens.refreshToken);
  });

  it('should handle invalid input', async () => {
    (loginSchema.safeParse as vi.Mock).mockReturnValue({ success: false, error: { format: () => 'Invalid input' } });
  ...
  it('should handle errors during login', async () => {
    (loginSchema.safeParse as vi.Mock).mockReturnValue({ success: true, data: { username: 'test', password: 'password' } });
    (AuthService.prototype.login as vi.Mock).mockRejectedValue(new Error('Test error'));
