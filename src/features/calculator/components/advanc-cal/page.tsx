...
export default function AdvancedCalculatorPage() {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent)_ => {
  ...
  const handleAllClear = useCallback(() => { handleClear(); setHistory([]); }, []);
  const handleBackspace = useCallback(() => { if (result) setResult(""); else setExpr(prev => prev.slice(0, -1)); }, [result]);
  const handleEvaluate = useCallback(() => { compute(expr); }, [compute, expr]);
