...
describe("CategorySelect", () => {
	it("should render with the initial category", () => {
		...
		expect(
			screen.getByText(CATEGORIES.find((c) => c.value === "length").label),
		).toBeInTheDocument();
	});
	...
	it("should open the select menu on click", () => {
		...
		expect(
			screen.getByText(CATEGORIES.find((c) => c.value === "weight").label),
		).toBeInTheDocument();
	});
	...
	it("should call setCategory when a new category is selected", () => {
		...
		const weightOption = screen.getByText(
			CATEGORIES.find((c) => c.value === "weight").label,
		);
