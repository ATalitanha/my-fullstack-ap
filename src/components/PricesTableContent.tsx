...
import { FaBitcoin, FaCoins, FaDollarSign, FaSearch } from "react-icons/fa";
...
export default function PricesTableContent() {
	const [data, setData] = useState<MarketData | null>(null);
	const [search, setSearch] = useState("");
	const [category, setCategory] = useState<Category>("all");
	const { t } = useTranslation("app.Prices-table");

	const categories: { value: Category; label: string }[] = [
		{ value: "all", label: t("categories.all") },
		{ value: "gold", label: t("categories.gold") },
		{ value: "currency", label: t("categories.currency") },
		{ value: "cryptocurrency", label: t("categories.cryptocurrency") },
	];
	...
	const getIcon = (itemCategory: Category) => {
		if (itemCategory === "cryptocurrency")
			return <FaBitcoin className="text-yellow-500 w-5 h-5" />;
		if (itemCategory === "currency")
			return <FaDollarSign className="text-green-500 w-5 h-5" />;
			...
			>
				...
				<div className="container mx-auto px-4 py-12 relative z-10">
                    ...
                    "
                      ...
                      "
										>
											<Select.Viewport className="p-2 h-full overflow-y-auto transition-all">
												{categories.map((cat) => (
													<Select.Item
														key={cat.value}
														value={cat.value}
					...
					>
						<AnimatePresence>
							{items.map((item, index) => {
								const itemCategory = data.gold.some(
